...
With the ab initio-corrected Hamiltonian and alpha extended to -2.0 eV, the seeded exciton run reproduces a clean self-trapping signature over the full 800 fs window, consistent with what the earlier methodology-development runs (Section 2.2.2) first established under the original hopping parameters. Hole and electron density co-localizing rather than separating is the correct signature of a bound exciton trapping itself, not charge transfer. One detail worth flagging rather than glossing over: the density maps show the localization occurring simultaneously in *two* separate site clusters (around atom indices ~13 and ~30, roughly opposite sides of the ring) rather than a single localized region — not yet understood, and not obviously an artifact given both the hole and electron maps agree on it independently. Whether this is a genuine two-polaron-like effect, a symmetry-related pairing (the two regions are close to diametrically opposite on the ring), or specific to this velocity seed is an open question for follow-up, not resolved here. Total energy is conserved to floating-point precision over the full 800 fs run (relative drift `-9.45e-14` at t=end, max `|drift|` `1.23e-12` Ha), ruling out numerical drift as a contributor to the continued growth late in the run — the self-trapping signature above is real dynamics.
...
One result here is worth reporting plainly rather than smoothing over, since it complicates the simplest possible "control just oscillates" story: a single bond (index ~53) develops a large, late-growing distortion that, by the end of the run, reaches a peak magnitude comparable to Run 2's broader exciton-driven distortion. It does not coincide spatially with either of Run 2's two exciton-localization sites (atom indices ~13 and ~30 — though a much weaker echo does appear near ~13 here), and it arises from the same random velocity seed acting on the bare lattice, with no exciton involved. The most likely reading is that this particular seed happens to drive a real, single-mode instability or resonance in the lattice's own anharmonic (repulsive-potential) dynamics, unrelated to the exciton — but this one control run does not establish that on its own; distinguishing "this seed happens to excite a real lattice mode regardless of the exciton" from any more exciton-adjacent explanation would need testing with independent velocity seeds, which hasn't been done here (see Section 2.6). What this run does establish cleanly is the comparison it was designed for: Run 2's *specific* result — hole and electron density co-localizing with lattice distortion at two particular sites, in the recurring beat pattern of Figure 9 — is not reproduced by the same velocity kick on the bare ground state.

Energy conservation has not yet been checked for this run (see `check_energy_conservation.py`); given Runs 1 and 2 both conserved energy to floating-point precision (~1e-13 relative) under the same integrator and timestep, it is expected to hold here too, but this has not been confirmed.
