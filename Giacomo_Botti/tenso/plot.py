...
def load_spinboson_pops(fname, convert_to_ps=False):
    ...
    if convert_to_ps:
        t *= 1e-3
    #pops = np.vstack((arr[:,1].real, arr[:,6].real)).T
    pops = np.vstack((arr[:,11].real, arr[:,16].real)).T
    return t, pops
...
# --- (a) populations ---
ax1.plot(t_sb, p_sb[:,0], label=r"$\rho_{11}$ TT")
ax1.plot(t_sb, p_sb[:,1], label=r"$\rho_{00}$ TT")
...
ax1.grid(True, which="major", color="0.8", linestyle="-", linewidth=0.8)
