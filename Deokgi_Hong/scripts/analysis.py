...
def plot_total_pop(times):
    ...
    for n_state in range(nstates):
        plt.plot(times*au_to_fs, el_pop[:, n_state], color=colors[n_state],
                 label='S' + str(n_state))
    plt.xlabel('Time (fs)', fontsize=medium_size)
...
#         adj_colors[istates[l]]="k"
        for n in range(numstates):
            if n == istates[line]:
                plt.plot(time[key]*au_to_fs, poten[key][:, n]*au_to_ev, color=colors[col],
...
def plot_total_energies(time, toten, keys):
    ...
    col = 0
    for key in keys:
        plt.plot(time[key]*au_to_fs, toten[key]*au_to_ev,
    ...
    plt.xlabel('Time (fs)', fontsize=medium_size)
    plt.ylabel('Total Energy (eV)', fontsize=medium_size)
...
def plot_nuclear_populations(ntraj, linestyles, labels, markers):
    ...
    for n in range(ntraj):
        plt.plot(qm_time*au_to_fs, N[:, n + 1], linestyle=linestyles[n],
